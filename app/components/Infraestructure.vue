<script setup lang="ts">
import { ref, onMounted, onUnmounted, computed } from 'vue'
import { useWindowSize } from '@vueuse/core'
import { useScrollAnimation } from "~/composables/useScrollAnimation"
import InteractiveNode from "./InteractiveNode/InteractiveNode.client.vue"

// 1. Import width to make logic responsive
const { width } = useWindowSize()
const { mapRange, easeInOutCubic } = useScrollAnimation()

// --- Logic ---
const showInfo = ref(false)
const sectionRef = ref<HTMLElement | null>(null)
let observer: IntersectionObserver | null = null

onMounted(() => {
    observer = new IntersectionObserver((entries) => {
        const entry = entries[0]

        if (entry!.isIntersecting) {
            setTimeout(() => {
                showInfo.value = true
            }, 1500)

            observer?.disconnect()
        }
    }, {
        threshold: 0.4
    })

    if (sectionRef.value) {
        observer.observe(sectionRef.value)
    }
})

onUnmounted(() => {
    observer?.disconnect()
})

// --- Dynamic Animation Config ---

const entranceStart = computed(() => width.value < 768 ? 500 : 500)
const entranceEnd = computed(() => width.value < 768 ? 700 : 900)
const exitStart = computed(() => width.value < 768 ? 900 : 1000)
const exitEnd = computed(() => width.value < 768 ? 1400 : 1700)


// 4. Pass the Computed Refs directly to mapRange
const progressEntrance = mapRange(entranceStart, entranceEnd, 0, 100, easeInOutCubic)
const progressExit = mapRange(exitStart, exitEnd, 100, 0, easeInOutCubic)

const progress = computed(() => Math.min(progressEntrance.value, progressExit.value))

</script>

<template>
    <div ref="sectionRef"
        class="mb-10 flex flex-col relative min-h-screen justify-center items-center overflow-hidden w-full">
        <ClientOnly>
            <InteractiveNode class="transition-opacity duration-500 ease-in-out cursor-move"
                :style="{ opacity: `${progress * (showInfo ? 1 : 0.3)}%` }" />

            <div class="mb-10 flex flex-col items-center transition-transform duration-1000 ease-in-out z-10 px-4 pointer-events-none select-none"
                :class="{ '-translate-y-66 md:-translate-y-76': showInfo }"
                :style="{ opacity: `${progressEntrance}%` }">

                <div class="flex items-center gap-4 mb-2 text-center">
                    <h1 class="tracking-wide text-xl md:text-2xl font-medium text-gray-100">
                        Infrastructure & Environment
                    </h1>
                </div>

                <div class="transition-all duration-500 ease-in-out max-w-lg text-center"
                    :class="showInfo ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'">
                    <p class="text-gray-400 font-light text-xs md:text-sm tracking-widest uppercase">
                        Interactive Topology &middot; Click nodes to inspect
                    </p>
                </div>
            </div>

            <template #fallback>
                <h1 class="font-medium tracking-wide text-2xl">
                    Infrastructure & Environment
                </h1>
            </template>
        </ClientOnly>
    </div>
</template>